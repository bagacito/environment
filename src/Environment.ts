...
function invalidEnvironmentVariable(
  ...
  const error = new Error(`Key ${envKey} expected ${expectedType} but received type string`)
...
class EnvironmentAccumulator<T extends object = never> extends ObjectAccumulator<T> {
  override accumulate<V extends object>(value: V): T & V & ObjectAccumulator<T & V> {
...
export const Environment = new EnvironmentAccumulator()
